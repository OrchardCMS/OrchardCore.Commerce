...
#pragma warning restore S107 // Methods should not have too many parameters
        ...
        Action<OrderPart> updateOrderPart = null)
    {
        ...
        {
            // AdditionalData only exists if Gross Price or a Promotion is specified.
            var additionalData = line.AdditionalData;
            var additionalDataExists = additionalData.Count > 0;
            if (additionalDataExists && additionalData.GetGrossPrice().Value > 0)
            {
                var grossPrice = additionalData.GetGrossPrice();
                grossTotal += grossPrice * line.Quantity;
            }

            var netPrice = additionalDataExists && additionalData.GetNetPrice().Value > 0
                ? additionalData.GetNetPrice()
                : line.UnitPrice;
