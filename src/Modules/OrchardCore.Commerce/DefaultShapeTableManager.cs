...
public class DefaultShapeTableManager : IShapeTableManager
{
    ...
    public DefaultShapeTableManager(
        [FromKeyedServices(nameof(DefaultShapeTableManager))]
        IDictionary<string, ShapeTable> shapeTableCache,
    ...
    public Task<ShapeTable> GetShapeTableAsync(string themeId) =>
        _shapeTableCache.TryGetValue(themeId ?? DefaultThemeIdKey, out var shapeTable)
            ? Task.FromResult(shapeTable)
            : BuildShapeTableAsync(themeId);
...
#pragma warning disable CA1308
            var key = bindingStrategy.GetType().Name
                      ...
                      + firstAlteration.ShapeType.ToLowerInvariant();
#pragma warning restore CA1308
    ...
    private static bool IsModuleOrRequestedTheme(IExtensionManager extensionManager, IFeatureInfo feature, string themeId)
    {
        ...
        bool IsBaseTheme(string themeFeatureId, string id) =>
            extensionManager
                .GetFeatureDependencies(id)
