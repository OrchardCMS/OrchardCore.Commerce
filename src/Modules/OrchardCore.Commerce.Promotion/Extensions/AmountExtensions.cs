...
public static class AmountExtensions
{
    public static Amount WithDiscount(this Amount amount, decimal discountPercentage) =>
        new(Math.Round(amount.Value * (1 - (discountPercentage / 100)), 2, Amount.MidpointRounding), amount.Currency);
    ...
    public static Amount WithDiscount(this Amount amount, Amount discountAmount)
    {
        ...
        return new(Math.Round(Math.Max(0, amount.Value - discountAmount.Value), 2, Amount.MidpointRounding), amount.Currency);
    }
}
