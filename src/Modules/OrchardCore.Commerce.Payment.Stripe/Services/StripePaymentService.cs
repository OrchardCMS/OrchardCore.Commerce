...
#pragma warning restore S107 // Methods should not have too many parameters
    ...
    public async Task<string> CreateClientSecretAsync(Amount total, ShoppingCartViewModel cart)
    {
        ...
        var paymentIntentId = await _paymentIntentPersistence.RetrieveAsync();
    ...
    {
        var innerPaymentIntentId = paymentIntentId ?? await _paymentIntentPersistence.RetrieveAsync();
        ...
        bool needToJudgeIntentStorage = true)
    {
        ...
        if (needToJudgeIntentStorage && string.IsNullOrEmpty(await _paymentIntentPersistence.RetrieveAsync()))
        {
        ...
        await _paymentIntentPersistence.RemoveAsync();
        return new PaymentOperationStatusViewModel
        {
            Status = PaymentOperationStatus.Failed,
            ShowMessage = H["The payment has failed, please try again."],
